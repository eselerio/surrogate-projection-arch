...
\title{Supplementary Material\\Enforcement of Mass Conservation and Non-Negativity in Statistical Surrogates of the Activated Sludge Process}
\author{}
\date{}
...
The timing results sharpen the accuracy findings. MLP delivered the best full-size accuracy with moderate fitting time, while CatBoost and SVR were slower to refit under the selected settings. The multi-task linear surrogates were extremely fast and benefited from projection, but their final nRMSE remained higher than the nonlinear alternatives. Within this benchmark, projection complemented the choice of a surrogate suited to the desired accuracy--training-cost tradeoff by adding a common constraint-enforcement step after prediction.
...
At $N=10{,}000$, complete 20-component raw inference ranged from approximately 0.001 ms per sample for PLS and the multi-task linear surrogates to 1.874 ms per sample for SVR (Figure~\ref{fig:s_inference_latency}). Projection-only latency ranged from 0.068 to 0.144 ms per sample across surrogates. The projection therefore dominated end-to-end latency for the fastest linear predictors but remained below 0.15 ms per sample under the stated batching protocol. All 766,350 production projections used the QR solver; the SVD fallback was not activated. Under this protocol, projection supplied deterministic mass-conservation and non-negativity enforcement at a small absolute computational cost, which may be useful in downstream screening, optimization, or decision-support loops that require admissible concentration vectors.
